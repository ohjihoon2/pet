...
<link rel="stylesheet" type="text/css"href="http://localhost:9090/pet/css/admin/admin_cat_vet.css">
...
$(document).ready(function(){
	...
	var pager = jQuery('#ampaginationsm').pagination({
	 ...
	 jQuery('#ampaginationsm').on('am.pagination.change',function(e){
           ...
           $(location).attr('href', "http://localhost:9090/pet/admin_cat_vet.do?rpage="+e.page);         
    }); 
});
...
<body>
	...
	<div id="content">
		<div id="vet">
			<h1 class="title">고양이 전문가팁 관리</h1>
			<div class="vet_btn">
				<a href="http://localhost:9090/pet/admin_cat_vet_write.do">
			...
			<table>
					...
					<tr>
						<td>${vo.rno}</td>
						<td class="table_title"><a href="http://localhost:9090/pet/admin_cat_vet_content.do?tip_id=${vo.tip_id}&page=${rpage}">${vo.tip_maintitle}</a></td>
						<td>${vo.tip_date}</td>
						<td>${vo.tip_kind}</td>
					</tr>
					</c:forEach>

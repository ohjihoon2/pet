...
<body>
	<div>
		<div id="content">
			...
			<form action="kakaoPay3.do" method="post" name="form">
			<div class="paycontent">
				...
				<table class="paydt">
						<tr>
							...
							<tr>
								<td>1</td>
								<td><img style="width:120px" class="item_simg" src="http://localhost:9090/pet/upload/${item_simg}"/></td>
								<td><span class="name">${item_name}</span></td>
								<td><fmt:formatNumber  value="${item_price}" pattern="#,##0원" /></td>
								<td>
									<input type=hidden class="item_price" name="item_price" value="${item_price}">
									<input type=hidden class="item_no" name="item_no" value="${item_no}">
									<input type=hidden class="qty" name="qty" value="${qty}">
									<span>${qty}</span>
								</td>
								<td><span class="sum"><fmt:formatNumber  value="${item_price * qty}" pattern="#,##0" /></span>원</td>
								<td>배송비 무료</td>
							</tr>
					</table>
					...
					<div class="final">
						<article>
							...
							<span>합계</span>
							<span>
							<input type="hidden" name="total" value="${item_price * qty}">
							<fmt:formatNumber value="${item_price * qty}" pattern="#,##0" />원
							</span>
						</article>
						...
						<article>
							<span>합계금액</span>
							<span style="display:inline-block;"><fmt:formatNumber  value="${item_price * qty}" pattern="#,##0" />원</span>

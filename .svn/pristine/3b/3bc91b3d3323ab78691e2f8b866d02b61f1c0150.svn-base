    ...
    function html5Upload() {	
    	var tempFile,
    		sUploadURL;
    	sUploadURL= '/pet/multiplePhotoUpload'; 	//upload URL
    	//파일을 하나씩 보내고, 결과를 받음.
...
//		alert("[가이드]사진 업로더할 서버URL셋팅이 필요합니다.-onAjaxError");
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	        ...
 	        sCallback : location.href.replace(/\/[^\/]*$/, '') + '/editor/photo_uploader/popup/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소

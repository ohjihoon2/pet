...
<link rel="stylesheet" type="text/css"href="http://localhost:9090/pet/css/admin/admin_friend.css">
...
<script >
function deleteWal(friend){
	 var result = confirm("해당 게시물을 삭제하시겠습니까?");
	if(result){
	    alert("게시물이 삭제되었습니다.");
	    location.href="admin_friend_delete_proc.do?wal_no="+friend;
	}else{
		location.href="http://localhost:9090/pet/admin_friend.do";
	}
}
$(document).ready(function(){
	...
	var pager = jQuery('#ampaginationsm').pagination({
	 ...
	 jQuery('#ampaginationsm').on('am.pagination.change',function(e){
           ...
           $(location).attr('href', "http://localhost:9090/pet/admin_friend.do?rpage="+e.page);         
    }); 
		...
		<div id = "walk_mgm">
			...
			<table>
				...
				<c:forEach var="vo" items="${list }">	
					<tr>
						<td>${vo.rno }</td>
						<td><img src="http://localhost:9090/pet/upload/${vo.wal_sfile}" style="width:280px;height: 200px;margin: 10px 0px;"></td>
						<td>${vo.wal_name}</td>
						<td>${vo.wal_dog}</td>
						<td>${vo.wal_age}</td>
						<td>${vo.mem_id}</td>
						<td>${vo.wal_date}</td>
						<td>${vo.wal_hits} 회</td>
						<td><a href="#" onclick="deleteWal('${vo.wal_no }')">삭제</a></td>
					</tr>
				</c:forEach>
			</table>
